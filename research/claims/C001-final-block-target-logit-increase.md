...
- Origin: exploratory
- Related experiments: Experiment 001, Experiment 002
...
The claim and its replication evidence do not extend beyond the 28 measured country–capital pairs, the two exact templates, this checkpoint, this projection metric, or these tokenization criteria. They do not extend to other facts, languages, models, model families, or causal mechanisms.
...
The intended capital token was not the actual top-1 prediction for any of the four Experiment 001 prompts or any of the 48 Experiment 002 prompt cases. This does not contradict the logit-change claim, but it sharply limits any interpretation of the pattern as successful factual recall.
...
Experiment 002 balanced target-token reuse within the tested candidate set and added a second template, but training-corpus frequency, non-capital vocabulary behavior, normalization-specific effects, and other model-wide transformations remain possible explanations.
...
- that the pattern generalizes beyond the 28 measured country–capital pairs, two exact templates, checkpoint, and projection metric;
- that the final block increases the correct target's support relative to balanced capital-token controls; Experiment 002 measured the opposite direction in all 48 prompt cases;
- that the model successfully produced an intended capital as top-1 in any Experiment 002 case.
...
- None of the four Experiment 001 targets or 48 Experiment 002 prompt cases produced the intended capital as the actual top-1 next token.
