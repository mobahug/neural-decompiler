"""The low-rank decompiled cue computation for Experiment 006, evaluated without the network.

Reads only the tensors exported under ``outputs/experiment-006/parameters/``: the token
embedding, block-0 second LayerNorm and MLP (the token-local encoder E), the final
LayerNorm, the unembedding, the PCA basis ``U_r`` with its centering mean, one map
``V_T`` per template, the reference cue ids, and the frozen context residuals. It never
imports TransformerLens, Transformers, or the instrumentation modules, and it never runs
attention, a block, or a prompt.

Coordinates (design revision 4):
    z(w)     = U_rᵀ (E(w) − μ_E)
...
class LowRankProgram:
    ...
    basis: torch.Tensor  # U_r [d_model, r]
    maps: Mapping[str, torch.Tensor]  # template -> V_T [d_model, r]
    reference_ids: Mapping[str, int]
    rho_frame: Mapping[str, torch.Tensor]
    rho_template: Mapping[str, torch.Tensor]

    @classmethod
    def load(cls, directory: Path) -> "LowRankProgram":
        ...
        return cls(
            eps=float(index["eps"]), rank=int(index["rank"]),
            ...
            mu=tensors["mu"], basis=tensors["basis"], maps={template: tensors[f"map.{template}"] for template in templates},
    ...
    def E(self, token_id: int) -> torch.Tensor:
        ...
        return torch.nn.functional.gelu(pre) @ self.mlp0_W_out + self.mlp0_b_out

    def z(self, token_id: int) -> torch.Tensor:
        return self.basis.T @ (self.E(token_id) - self.mu)

    def dz(self, template: str, token_id: int) -> torch.Tensor:
        return self.basis.T @ (self.E(token_id) - self.E(self.reference_ids[template]))
    ...
    def predict_epatch_shift(self, template: str, frame_id: str | None, token_id: int, sg_id: int, pl_id: int) -> float:
        ...
        delta = self.maps[template] @ self.dz(template, token_id)
