#!/usr/bin/env python3
"""Run Experiment 015 through isolated phases.

``validate`` checks the frozen inputs without a model. ``freeze-confirmation`` builds ``confirmation-v1.json`` from
tokenizer rules (refuses to overwrite). ``explore`` (once) captures the 48 exposed frames' reference states (the
residuals before blocks 1 and 2 at every position up to the cue and the layer-1–2 pattern rows), re-measures Experiment
014's 4884 recorded pairs with the patched residuals and pattern rows captured, replicates the 014 ledger and the 013
pattern extract, checks the Level-1 identities, and records the exposed statistics of the token-local cue-change model,
its axis-only alternative and the comparators. ``lock`` computes the prediction table for the fresh tokens in the 48
exposed frames from the weights, the Experiment 011/012 locks and the locked reference states — no forward pass, no
capture or intervention call path — and writes the candidate lock and the prediction artifact. ``confirm`` (once)
validates the artifacts, reproduces every locked prediction, then runs stage 1 (the fresh frames' reference prompts,
validity, and the frame-conditional prediction table, digested into the results state) and, only after re-reading that
digest from disk, stage 2 (every fresh cue in both sets, scored against the two tables). ``report`` renders the report.
"""
...
OUTPUT_DIR = ROOT / "outputs/experiment-015"
...
def build_parser() -> argparse.ArgumentParser:
    ...
    for phase, help_text in (("validate", "check the frozen inputs and the confirmation set without a model"), ("freeze-confirmation", "build confirmation-v1.json from tokenizer rules only (refuses to overwrite)"),
                             ("explore", "Tier A: reference states, re-measurement, replication, identities, exposed statistics"), ("lock", "compute the prediction table for fresh tokens × exposed frames; write the candidate lock and predictions"),
                             ("confirm", "Tier C: stage 1 (fresh frames' reference states, digested predictions) then stage 2 (fresh cues, scoring)"), ("report", "render outputs/experiment-015/report.md")):
...
def runtime_record(spec: ModelSpec) -> dict[str, Any]:
    ...
    return {"device": runtime.device, "backend": runtime.backend, "dtype": runtime.dtype_name, "seed": atp.RUNTIME_SEED, "control_seed": atp.CONTROL_SEED, "deterministic_algorithms": spec.deterministic_algorithms,
...
def _load_lock(path: Path, experiment: str) -> dict[str, Any]:
    ...
    if lock.get("experiment") != experiment or lock.get("content_sha256") != pm.sha256_text(pm.canonical_json(unsigned)):
        raise atp.PhaseError(f"the Experiment {experiment} lock's content digest does not verify")
    return lock
...
class Runner:
    ...
    def confirmation_path(self) -> Path:
        return self.root / atp.CONFIRMATION_RELATIVE_PATH
    ...
    def _base_inputs(self):
        ...
        if confirmation_006.content_sha256 != ss.INHERITED_CONFIRMATION_SHA256:
            raise atp.PhaseError("the Experiment 006 confirmation set digest is not the frozen constant")
        ...
        for relative in (cd.CONFIRMATION_RELATIVE_PATH, ht.CONFIRMATION_RELATIVE_PATH, er.CONFIRMATION_RELATIVE_PATH, atp.EXPERIMENT_011_LOCK_PATH, lc.CONFIRMATION_RELATIVE_PATH, atp.EXPERIMENT_012_LOCK_PATH,
                         atp.EXPERIMENT_013_CONFIRMATION_PATH, atp.EXPERIMENT_013_LOCK_PATH, atp.EXPERIMENT_014_CONFIRMATION_PATH, atp.EXPERIMENT_014_LOCK_PATH, atp.INHERITED_014_LEDGER_RELATIVE_PATH, atp.INHERITED_013_EXTRACT_RELATIVE_PATH):
            if not (self.root / relative).exists() or not self.tracked(self.root / relative):
                raise atp.PhaseError(f"{relative} must exist and be tracked and committed")
        lock_011 = self.lock_011_loader(self.root / atp.EXPERIMENT_011_LOCK_PATH)
        if any(key not in lock_011 for key in LOCK_011_REQUIRED_KEYS) or lock_011["confirmation_011_sha256"] != confirmation_011.content_sha256:
            raise atp.PhaseError("the Experiment 011 lock does not carry the locked axes and read weight for the frozen 011 confirmation set")
        ...
        lock_012 = self.lock_012_loader(self.root / atp.EXPERIMENT_012_LOCK_PATH)
        if any(key not in lock_012 for key in LOCK_012_REQUIRED_KEYS) or lock_012["confirmation_012_sha256"] != confirmation_012.content_sha256 or lock_012["lock_011_sha256"] != lock_011["content_sha256"]:
            raise atp.PhaseError("the Experiment 012 lock does not carry the frozen model for the frozen 012 confirmation set and the 011 lock")
        ...
        confirmation_013 = ap.load_confirmation(self.root / atp.EXPERIMENT_013_CONFIRMATION_PATH, pool_013, digests)
        digests["confirmation_013"] = confirmation_013.content_sha256
        lock_013 = self.lock_013_loader(self.root / atp.EXPERIMENT_013_LOCK_PATH)
        if any(key not in lock_013 for key in LOCK_013_REQUIRED_KEYS) or lock_013["confirmation_013_sha256"] != confirmation_013.content_sha256 or lock_013["lock_012_sha256"] != lock_012["content_sha256"]:
            raise atp.PhaseError("the Experiment 013 lock does not carry the locked reference states for the frozen 013 confirmation set and the 012 lock")
        ...
        confirmation_014 = nf.load_confirmation(self.root / atp.EXPERIMENT_014_CONFIRMATION_PATH, pool_014, digests)
        digests["confirmation_014"] = confirmation_014.content_sha256
        lock_014 = self.lock_014_loader(self.root / atp.EXPERIMENT_014_LOCK_PATH)
        if any(key not in lock_014 for key in LOCK_014_REQUIRED_KEYS) or lock_014["confirmation_014_sha256"] != confirmation_014.content_sha256 or lock_014["lock_013_sha256"] != lock_013["content_sha256"]:
            raise atp.PhaseError("the Experiment 014 lock does not carry the locked reference states for the frozen 014 confirmation set and the 013 lock")
        ...
        ledger = atp.load_inherited_ledger(self.root / atp.INHERITED_014_LEDGER_RELATIVE_PATH, digests=digests, expected_size=atp.EXPECTED_LEDGER_SIZE_014)
        digests["ledger_014"] = ledger["content_sha256"]
        extract = atp.load_inherited_extract(self.root / atp.INHERITED_013_EXTRACT_RELATIVE_PATH, digests=digests, expected_size=atp.EXPECTED_EXTRACT_SIZE_013)
        digests["extract_013"] = extract["content_sha256"]
        return pool, pool_010, lock_011, lock_012, lock_014, ledger, extract, digests

    def _inputs(self):
        pool, pool_010, lock_011, lock_012, lock_014, ledger, extract, digests = self._base_inputs()
        if not self.confirmation_path.exists() or not self.tracked(self.confirmation_path):
            raise atp.PhaseError("the Experiment 015 confirmation set must be frozen and committed before any scientific phase")
        confirmation = atp.load_confirmation(self.confirmation_path, pool, digests)
        digests = dict(digests) | {"confirmation_015": confirmation.content_sha256}
        return pool, pool_010, lock_011, lock_012, lock_014, ledger, extract, confirmation, digests

    def validate(self) -> int:
        try:
            pool, pool_010, lock_011, lock_012, lock_014, ledger, extract, digests = self._base_inputs()
        except (atp.PhaseError, ValueError) as error:
            self.log(f"validation failed: {error}")
            return 1
        self.log(f"exposed pool: {len(pool.frames)} frames, {len(pool.tokens)} tokens; 014 ledger {len(ledger['entries'])} pairs; 013 pattern extract {len(extract['entries'])} pairs; 014 lock {digests['lock_014'][:12]}…")
    ...
    def freeze_confirmation(self) -> int:
        ...
        if self.confirmation_path.exists():
            ...
            return 1
        digest = atp.freeze_confirmation(self.confirmation_path, self.tokenizer_loader(PYTHIA_70M), pool, digests)
    ...
    def _state_for(self, phase: str, digests: Mapping[str, str]) -> dict[str, Any]:
        ...
        if self.results_path.exists():
            state = atp.load_results_state(self.results_path)
            if provenance["git_dirty"] or not pm._COMMIT_SHA.fullmatch(provenance["protocol_code_commit"]):
                raise atp.PhaseError("scientific execution requires a clean Git tree at a committed SHA")
            if atp.state_digests(state) != atp.digest_tuple(digests) or dict(state["versions"]) != provenance["versions"]:
                raise atp.PhaseError("frozen inputs or dependency versions differ from the recorded run")
        else:
            if phase != "explore":
                raise atp.PhaseError(f"{phase} requires an existing results state from explore")
            state = atp.new_results_state(digests=digests, **provenance)
        atp.assert_phase_allowed(phase, state)
        return state
    ...
    def _record_incident(self, state: dict[str, Any], phase: str, error: Exception) -> None:
        ...
        atp.write_results_state(self.results_path, state)
        self.log(f"INCIDENT ({phase}): {error}")

    def explore(self) -> int:
        pool, pool_010, lock_011, lock_012, lock_014, ledger, extract, confirmation, digests = self._inputs()
        ...
        if incidents and incidents[-1]["commit"] == commit:
            raise atp.PhaseError("an explore incident is recorded at this commit; a committed fix or documented amendment is required before explore runs again")
        ...
        atp.assert_confirmation_untouched(state, confirmation)
        atp.write_results_state(self.results_path, state)
        seed_runtime(atp.RUNTIME_SEED, PYTHIA_70M.deterministic_algorithms)
        model = self.model_loader(PYTHIA_70M)
        try:
            try:
                atp.run_exploration(model, pool, pool_010, lock_011=lock_011, lock_012=lock_012, lock_014=lock_014, inherited_ledger=ledger, inherited_extract=extract, state=state, results_path=self.results_path, log=self.log)
        ...
        digest = atp.write_results_state(self.results_path, state)
        summary = state["exploration"]["summary"]
        f = cd._f
        self.log(f"explore complete: exposed Spearman {f(summary['spearman'], 3)}, R² {f(summary['r2'], 3)}, entry R² {f(summary['entry_r2_1'], 3)} / {f(summary['entry_r2_2'], 3)}; axis-only R² {f(summary['axis_r2'], 3)}, entries {f(summary['axis_entry_r2'], 3)}; results sha256 {digest}")
        return 0
    ...
    def _weights_only(self) -> tuple[pm.Weights, lc.LayerWeights, dict[int, atp.LayerProgram]]:
        ...
        model = self.model_loader(PYTHIA_70M)
        try:
            return pm.Weights.from_model(model), lc.LayerWeights.from_model(model), atp.programs_from_model(model)
        finally:
            del model
            gc.collect()

    def _predictions(self, weights: pm.Weights, lw: lc.LayerWeights, programs: Mapping[int, atp.LayerProgram], source: Mapping[str, Any], lock_012: Mapping[str, Any], confirmation: atp.Confirmation015, pool: Any) -> dict[str, Any]:
        """``source`` is the exploration record at lock time and the lock itself at confirm time."""
        plural_ids = {template: pool.token_id(name) for template, name in pool.plural_cue.items()}
        tlm = atp.model_from_locks(source, lock_012, lw, programs, pool.reference_ids, plural_ids)
        return atp.lock_predictions(tlm, weights, source["locked_states"], pool, confirmation, source["defined_templates"])
    ...
    def lock(self) -> int:
        ...
        if changed is None:
            raise atp.PhaseError("the explore commit is not an ancestor of the current commit; lock must be written at the explore protocol")
        scientific = [path for path in changed if path.startswith(atp.SCIENTIFIC_PATH_PREFIXES) and path not in atp.NON_SCIENTIFIC_PATHS and not path.startswith(atp.NON_SCIENTIFIC_PREFIXES)]
        if scientific:
            raise atp.PhaseError(f"scientific paths changed since explore: {scientific}; lock must be written at the explore protocol")
        ...
        lock = atp.build_candidate_lock(state=state, digests=digests, confirmation=confirmation, predictions=predictions, protocol_code_commit=self._provenance()["protocol_code_commit"])
        ...
        predictions_text = atp.render_predictions(lock)
        ...
        atp.write_results_state(self.results_path, state)
        self.log(f"candidate lock written to {candidate_path} (sha256 {lock['content_sha256']}) and {predictions_path} (sha256 {state['lock']['predictions_sha256']}). Install both as {atp.LOCK_RELATIVE_PATH} and {atp.PREDICTIONS_RELATIVE_PATH} and commit before confirm.")
        return 0

    def confirm(self) -> int:
        pool, pool_010, lock_011, lock_012, lock_014, ledger, extract, confirmation, digests = self._inputs()
        state = self._state_for("confirm", digests)
        lock_path, predictions_path = self.root / atp.LOCK_RELATIVE_PATH, self.root / atp.PREDICTIONS_RELATIVE_PATH
        if not lock_path.exists() or not predictions_path.exists():
            raise atp.PhaseError("missing committed lock or predictions artifact")
        lock = json.loads(lock_path.read_text(encoding="utf-8"))
        git = self.git_state()
        atp.validate_lock(lock, state=state, digests=digests, confirmation=confirmation, predictions_text=predictions_path.read_text(encoding="utf-8"), git_state=git,
        ...
        if recorded_runtime is not None and {k: v for k, v in runtime.items() if k != "seed"} != {k: v for k, v in recorded_runtime.items() if k != "seed"}:
            raise atp.PhaseError(f"the confirm runtime {runtime} differs from the explore runtime {recorded_runtime}; the re-captured reference states must be bitwise reproducible")
        seed_runtime(atp.RUNTIME_SEED, PYTHIA_70M.deterministic_algorithms)
        model = self.model_loader(PYTHIA_70M)
        try:
            weights, lw, programs = pm.Weights.from_model(model), lc.LayerWeights.from_model(model), atp.programs_from_model(model)
            reproduced = atp.assert_lock_predictions_reproduced(lock, self._predictions(weights, lw, programs, lock, lock_012, confirmation, pool))  # PhaseError before anything fresh runs
            ...
            atp.write_results_state(self.results_path, state)
            try:
                stage1 = atp.stage_one(model, pool, pool_010, confirmation, lock, lock_012, protocol_code_commit=commit, log=self.log)
                ...
                atp.write_results_state(self.results_path, state)
                results = atp.stage_two(model, pool, pool_010, confirmation, lock, lock_012, state["confirmation"]["stage1"], log=self.log)
            except pm.IncidentError as error:
    ...
    def report(self) -> int:
        self._inputs()
        state = atp.load_results_state(self.results_path)
        atp.assert_phase_allowed("report", state)
        self.report_path.parent.mkdir(parents=True, exist_ok=True)
        self.report_path.write_text(atp.render_report(state), encoding="utf-8")
