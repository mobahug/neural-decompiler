"""Guarded launcher for Experiment 024's production `calibrate` (run exactly once).
...
def _hook(event, args):
    ...
    if text.endswith("calibration-table.pt") or "/outputs/experiment-023/" in text:
        ...
        raise PermissionError(f"calibrate may not open {text}")
    ...
    if writing and text.startswith(str(ROOT)) and "/.venv/" not in text:
        ...
        raise PermissionError(f"calibrate may not write {text}")
...
def _guarded_call(self, *args, **kwargs):
    ...
    raise RuntimeError("a torch module was called during calibrate after the weights were read")
...
def _refuse_capture(*args, **kwargs):
    ...
    raise RuntimeError("a capture or intervention entry point was reached during calibrate")
...
(HERE / "calibrate_run.json").write_text(json.dumps(record, indent=1) + "\n")
