#!/usr/bin/env python3
"""Run Experiment 014 through isolated phases.

``validate`` checks the frozen inputs without a model. ``freeze-confirmation`` builds ``confirmation-v1.json`` from
tokenizer rules (refuses to overwrite). ``explore`` (once) captures the 42 exposed frames' reference states,
re-measures Experiment 013's 3732 recorded pairs with the residual before block 2 captured, replicates the 013 ledger,
and records the exposed statistics of the neuron-1987 predictor and its axis-only alternative. ``lock`` computes the
prediction table for the fresh tokens in the 42 exposed frames from the weights, the Experiment 011/012 locks and the
locked reference states — no forward pass, no capture or intervention call path — and writes the candidate lock and
the prediction artifact. ``confirm`` (once) validates the artifacts, reproduces every locked prediction, then runs
stage 1 (the fresh frames' reference prompts, validity, and the frame-conditional prediction table, digested into the
results state) and, only after re-reading that digest from disk, stage 2 (every fresh cue in both sets, scored against
the two tables). ``report`` renders the report.
"""
...
OUTPUT_DIR = ROOT / "outputs/experiment-014"
...
def build_parser() -> argparse.ArgumentParser:
    ...
    for phase, help_text in (("validate", "check the frozen inputs and the confirmation set without a model"), ("freeze-confirmation", "build confirmation-v1.json from tokenizer rules only (refuses to overwrite)"),
                             ("explore", "Tier A: reference states, re-measurement, replication, exposed statistics"), ("lock", "compute the prediction table for fresh tokens × exposed frames; write the candidate lock and predictions"),
                             ("confirm", "Tier C: stage 1 (fresh frames' reference states, digested predictions) then stage 2 (fresh cues, scoring)"), ("report", "render outputs/experiment-014/report.md")):
...
def runtime_record(spec: ModelSpec) -> dict[str, Any]:
    ...
    return {"device": runtime.device, "backend": runtime.backend, "dtype": runtime.dtype_name, "seed": nf.RUNTIME_SEED, "control_seed": nf.CONTROL_SEED, "deterministic_algorithms": spec.deterministic_algorithms,
...
def _load_lock(path: Path, experiment: str) -> dict[str, Any]:
    ...
    if lock.get("experiment") != experiment or lock.get("content_sha256") != pm.sha256_text(pm.canonical_json(unsigned)):
        raise nf.PhaseError(f"the Experiment {experiment} lock's content digest does not verify")
    return lock
...
class Runner:
    ...
    def confirmation_path(self) -> Path:
        return self.root / nf.CONFIRMATION_RELATIVE_PATH
    ...
    def _base_inputs(self):
        ...
        if confirmation_006.content_sha256 != ss.INHERITED_CONFIRMATION_SHA256:
            raise nf.PhaseError("the Experiment 006 confirmation set digest is not the frozen constant")
        ...
        for relative in (cd.CONFIRMATION_RELATIVE_PATH, ht.CONFIRMATION_RELATIVE_PATH, er.CONFIRMATION_RELATIVE_PATH, nf.EXPERIMENT_011_LOCK_PATH, lc.CONFIRMATION_RELATIVE_PATH, nf.EXPERIMENT_012_LOCK_PATH,
                         nf.EXPERIMENT_013_CONFIRMATION_PATH, nf.EXPERIMENT_013_LOCK_PATH, nf.INHERITED_013_LEDGER_RELATIVE_PATH):
            if not (self.root / relative).exists() or not self.tracked(self.root / relative):
                raise nf.PhaseError(f"{relative} must exist and be tracked and committed")
        lock_011 = self.lock_011_loader(self.root / nf.EXPERIMENT_011_LOCK_PATH)
        if any(key not in lock_011 for key in LOCK_011_REQUIRED_KEYS) or lock_011["confirmation_011_sha256"] != confirmation_011.content_sha256:
            raise nf.PhaseError("the Experiment 011 lock does not carry the locked axes and read weight for the frozen 011 confirmation set")
        ...
        lock_012 = self.lock_012_loader(self.root / nf.EXPERIMENT_012_LOCK_PATH)
        if any(key not in lock_012 for key in LOCK_012_REQUIRED_KEYS) or lock_012["confirmation_012_sha256"] != confirmation_012.content_sha256 or lock_012["lock_011_sha256"] != lock_011["content_sha256"]:
            raise nf.PhaseError("the Experiment 012 lock does not carry the frozen model for the frozen 012 confirmation set and the 011 lock")
        ...
        confirmation_013 = ap.load_confirmation(self.root / nf.EXPERIMENT_013_CONFIRMATION_PATH, pool_013, digests)
        digests["confirmation_013"] = confirmation_013.content_sha256
        lock_013 = self.lock_013_loader(self.root / nf.EXPERIMENT_013_LOCK_PATH)
        if any(key not in lock_013 for key in LOCK_013_REQUIRED_KEYS) or lock_013["confirmation_013_sha256"] != confirmation_013.content_sha256 or lock_013["lock_012_sha256"] != lock_012["content_sha256"]:
            raise nf.PhaseError("the Experiment 013 lock does not carry the locked reference states for the frozen 013 confirmation set and the 012 lock")
        ...
        ledger = nf.load_inherited_ledger(self.root / nf.INHERITED_013_LEDGER_RELATIVE_PATH, digests=digests, expected_size=nf.EXPECTED_LEDGER_SIZE)
        digests["ledger_013"] = ledger["content_sha256"]
        return pool, pool_010, lock_011, lock_012, lock_013, ledger, digests

    def _inputs(self):
        pool, pool_010, lock_011, lock_012, lock_013, ledger, digests = self._base_inputs()
        if not self.confirmation_path.exists() or not self.tracked(self.confirmation_path):
            raise nf.PhaseError("the Experiment 014 confirmation set must be frozen and committed before any scientific phase")
        confirmation = nf.load_confirmation(self.confirmation_path, pool, digests)
        digests = dict(digests) | {"confirmation_014": confirmation.content_sha256}
        return pool, pool_010, lock_011, lock_012, lock_013, ledger, confirmation, digests

    def validate(self) -> int:
        try:
            pool, pool_010, lock_011, lock_012, lock_013, ledger, digests = self._base_inputs()
        except (nf.PhaseError, ValueError) as error:
            self.log(f"validation failed: {error}")
            return 1
        self.log(f"exposed pool: {len(pool.frames)} frames, {len(pool.tokens)} tokens; 013 ledger {len(ledger['entries'])} pairs; 013 lock {digests['lock_013'][:12]}…")
    ...
    def freeze_confirmation(self) -> int:
        ...
        if self.confirmation_path.exists():
            ...
            return 1
        digest = nf.freeze_confirmation(self.confirmation_path, self.tokenizer_loader(PYTHIA_70M), pool, digests)
    ...
    def _state_for(self, phase: str, digests: Mapping[str, str]) -> dict[str, Any]:
        ...
        if self.results_path.exists():
            state = nf.load_results_state(self.results_path)
            if provenance["git_dirty"] or not pm._COMMIT_SHA.fullmatch(provenance["protocol_code_commit"]):
                raise nf.PhaseError("scientific execution requires a clean Git tree at a committed SHA")
            if nf.state_digests(state) != nf.digest_tuple(digests) or dict(state["versions"]) != provenance["versions"]:
                raise nf.PhaseError("frozen inputs or dependency versions differ from the recorded run")
        else:
            if phase != "explore":
                raise nf.PhaseError(f"{phase} requires an existing results state from explore")
            state = nf.new_results_state(digests=digests, **provenance)
        nf.assert_phase_allowed(phase, state)
        return state
    ...
    def _record_incident(self, state: dict[str, Any], phase: str, error: Exception) -> None:
        ...
        nf.write_results_state(self.results_path, state)
        self.log(f"INCIDENT ({phase}): {error}")

    def explore(self) -> int:
        pool, pool_010, lock_011, lock_012, lock_013, ledger, confirmation, digests = self._inputs()
        ...
        if incidents and incidents[-1]["commit"] == commit:
            raise nf.PhaseError("an explore incident is recorded at this commit; a committed fix or documented amendment is required before explore runs again")
        ...
        nf.assert_confirmation_untouched(state, confirmation)
        nf.write_results_state(self.results_path, state)
        seed_runtime(nf.RUNTIME_SEED, PYTHIA_70M.deterministic_algorithms)
        model = self.model_loader(PYTHIA_70M)
        try:
            try:
                nf.run_exploration(model, pool, pool_010, lock_011=lock_011, lock_012=lock_012, lock_013=lock_013, inherited=ledger, state=state, results_path=self.results_path, log=self.log)
        ...
        digest = nf.write_results_state(self.results_path, state)
        summary = state["exploration"]["summary"]
        f = cd._f
        self.log(f"explore complete: exposed Spearman {f(summary['spearman'], 3)}, R² {f(summary['r2'], 3)}, balanced accuracy {f(summary['balanced_accuracy'], 3)}; axis-only R² {f(summary['axis_r2'], 3)}, balanced accuracy {f(summary['axis_balanced_accuracy'], 3)}; results sha256 {digest}")
        return 0
    ...
    def _predictions(self, weights: pm.Weights, lw: lc.LayerWeights, heads: ap.HeadSet, source: Mapping[str, Any], lock_012: Mapping[str, Any], confirmation: nf.Confirmation014, pool: Any) -> dict[str, Any]:
        ...
        neuron = nf.NeuronWeights.from_layer_weights(lw, fpm.read)
        d_E = torch.tensor(source["axes_vectors"]["R0"], dtype=torch.float64)
        return nf.lock_predictions(neuron, fpm, weights, source["locked_states"], pool, confirmation, source["defined_templates"], d_E)
    ...
    def lock(self) -> int:
        ...
        if changed is None:
            raise nf.PhaseError("the explore commit is not an ancestor of the current commit; lock must be written at the explore protocol")
        scientific = [path for path in changed if path.startswith(nf.SCIENTIFIC_PATH_PREFIXES) and path not in nf.NON_SCIENTIFIC_PATHS and not path.startswith(nf.NON_SCIENTIFIC_PREFIXES)]
        if scientific:
            raise nf.PhaseError(f"scientific paths changed since explore: {scientific}; lock must be written at the explore protocol")
        ...
        lock = nf.build_candidate_lock(state=state, digests=digests, confirmation=confirmation, predictions=predictions, protocol_code_commit=self._provenance()["protocol_code_commit"])
        ...
        predictions_text = nf.render_predictions(lock)
        ...
        nf.write_results_state(self.results_path, state)
        self.log(f"candidate lock written to {candidate_path} (sha256 {lock['content_sha256']}) and {predictions_path} (sha256 {state['lock']['predictions_sha256']}). Install both as {nf.LOCK_RELATIVE_PATH} and {nf.PREDICTIONS_RELATIVE_PATH} and commit before confirm.")
        return 0

    def confirm(self) -> int:
        pool, pool_010, lock_011, lock_012, lock_013, ledger, confirmation, digests = self._inputs()
        state = self._state_for("confirm", digests)
        lock_path, predictions_path = self.root / nf.LOCK_RELATIVE_PATH, self.root / nf.PREDICTIONS_RELATIVE_PATH
        if not lock_path.exists() or not predictions_path.exists():
            raise nf.PhaseError("missing committed lock or predictions artifact")
        lock = json.loads(lock_path.read_text(encoding="utf-8"))
        git = self.git_state()
        nf.validate_lock(lock, state=state, digests=digests, confirmation=confirmation, predictions_text=predictions_path.read_text(encoding="utf-8"), git_state=git,
        ...
        if recorded_runtime is not None and {k: v for k, v in runtime.items() if k != "seed"} != {k: v for k, v in recorded_runtime.items() if k != "seed"}:
            raise nf.PhaseError(f"the confirm runtime {runtime} differs from the explore runtime {recorded_runtime}; the re-captured reference states must be bitwise reproducible")
        seed_runtime(nf.RUNTIME_SEED, PYTHIA_70M.deterministic_algorithms)
        model = self.model_loader(PYTHIA_70M)
        try:
            weights, lw, heads = pm.Weights.from_model(model), lc.LayerWeights.from_model(model), ap.HeadSet.from_model(model)
            reproduced = nf.assert_lock_predictions_reproduced(lock, self._predictions(weights, lw, heads, lock, lock_012, confirmation, pool))  # PhaseError before anything fresh runs
            ...
            nf.write_results_state(self.results_path, state)
            try:
                stage1 = nf.stage_one(model, pool, pool_010, confirmation, lock, lock_012, protocol_code_commit=commit, log=self.log)
                ...
                nf.write_results_state(self.results_path, state)
                results = nf.stage_two(model, pool, pool_010, confirmation, lock, lock_012, state["confirmation"]["stage1"], log=self.log)
            except pm.IncidentError as error:
    ...
    def report(self) -> int:
        self._inputs()
        state = nf.load_results_state(self.results_path)
        nf.assert_phase_allowed("report", state)
        self.report_path.parent.mkdir(parents=True, exist_ok=True)
        self.report_path.write_text(nf.render_report(state), encoding="utf-8")
