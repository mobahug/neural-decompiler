#!/usr/bin/env python3
"""Run Experiment 016 through isolated phases.

``validate`` checks the frozen inputs without a model. ``freeze-confirmation`` builds ``confirmation-v1.json`` from
tokenizer rules (refuses to overwrite). ``explore`` (once) captures the 54 exposed frames' reference states, re-measures
Experiment 015's 6180 recorded pairs with the patched residuals and pattern rows captured, replicates the 015 extract,
checks the identities and the channel-switch recoveries, and records the exposed statistics of the frame-conditioned
token-local model, its scale-only alternative, the ablation ladder and the comparators. ``lock`` computes the prediction
table for the fresh tokens in the 54 exposed frames from the weights, the Experiment 011/012 locks and the locked
reference states — no forward pass, no capture or intervention call path — and writes the candidate lock and the
prediction artifact. ``confirm`` (once)
...
OUTPUT_DIR = ROOT / "outputs/experiment-016"
...
def build_parser() -> argparse.ArgumentParser:
    ...
    for phase, help_text in (("validate", "check the frozen inputs and the confirmation set without a model"), ("freeze-confirmation", "build confirmation-v1.json from tokenizer rules only (refuses to overwrite)"),
                             ("explore", "Tier A: reference states, re-measurement, replication, identities and recoveries, exposed statistics"), ("lock", "compute the prediction table for fresh tokens × exposed frames; write the candidate lock and predictions"),
                             ("confirm", "Tier C: stage 1 (fresh frames' reference states, digested predictions) then stage 2 (fresh cues, scoring)"), ("report", "render outputs/experiment-016/report.md")):
...
def runtime_record(spec: ModelSpec) -> dict[str, Any]:
    ...
    return {"device": runtime.device, "backend": runtime.backend, "dtype": runtime.dtype_name, "seed": fch.RUNTIME_SEED, "control_seed": fch.CONTROL_SEED, "deterministic_algorithms": spec.deterministic_algorithms,
...
def _load_lock(path: Path, experiment: str) -> dict[str, Any]:
    ...
    if lock.get("experiment") != experiment or lock.get("content_sha256") != pm.sha256_text(pm.canonical_json(unsigned)):
        raise fch.PhaseError(f"the Experiment {experiment} lock's content digest does not verify")
    return lock
...
class Runner:
    ...
    def confirmation_path(self) -> Path:
        return self.root / fch.CONFIRMATION_RELATIVE_PATH
    ...
    def _base_inputs(self):
        ...
        if confirmation_006.content_sha256 != ss.INHERITED_CONFIRMATION_SHA256:
            raise fch.PhaseError("the Experiment 006 confirmation set digest is not the frozen constant")
        ...
        for relative in (cd.CONFIRMATION_RELATIVE_PATH, ht.CONFIRMATION_RELATIVE_PATH, er.CONFIRMATION_RELATIVE_PATH, fch.EXPERIMENT_011_LOCK_PATH, lc.CONFIRMATION_RELATIVE_PATH, fch.EXPERIMENT_012_LOCK_PATH,
                         fch.EXPERIMENT_013_CONFIRMATION_PATH, fch.EXPERIMENT_013_LOCK_PATH, fch.EXPERIMENT_014_CONFIRMATION_PATH, fch.EXPERIMENT_014_LOCK_PATH, fch.EXPERIMENT_015_CONFIRMATION_PATH, fch.EXPERIMENT_015_LOCK_PATH,
                         fch.INHERITED_015_EXTRACT_RELATIVE_PATH):
            if not (self.root / relative).exists() or not self.tracked(self.root / relative):
                raise fch.PhaseError(f"{relative} must exist and be tracked and committed")
        lock_011 = self.lock_011_loader(self.root / fch.EXPERIMENT_011_LOCK_PATH)
        if any(key not in lock_011 for key in LOCK_011_REQUIRED_KEYS) or lock_011["confirmation_011_sha256"] != confirmation_011.content_sha256:
            raise fch.PhaseError("the Experiment 011 lock does not carry the locked axes and read weight for the frozen 011 confirmation set")
        ...
        lock_012 = self.lock_012_loader(self.root / fch.EXPERIMENT_012_LOCK_PATH)
        if any(key not in lock_012 for key in LOCK_012_REQUIRED_KEYS) or lock_012["confirmation_012_sha256"] != confirmation_012.content_sha256 or lock_012["lock_011_sha256"] != lock_011["content_sha256"]:
            raise fch.PhaseError("the Experiment 012 lock does not carry the frozen model for the frozen 012 confirmation set and the 011 lock")
        ...
        confirmation_013 = ap.load_confirmation(self.root / fch.EXPERIMENT_013_CONFIRMATION_PATH, pool_013, digests)
        digests["confirmation_013"] = confirmation_013.content_sha256
        lock_013 = self.lock_013_loader(self.root / fch.EXPERIMENT_013_LOCK_PATH)
        if any(key not in lock_013 for key in LOCK_013_REQUIRED_KEYS) or lock_013["confirmation_013_sha256"] != confirmation_013.content_sha256 or lock_013["lock_012_sha256"] != lock_012["content_sha256"]:
            raise fch.PhaseError("the Experiment 013 lock does not carry the locked reference states for the frozen 013 confirmation set and the 012 lock")
        ...
        confirmation_014 = nf.load_confirmation(self.root / fch.EXPERIMENT_014_CONFIRMATION_PATH, pool_014, digests)
        digests["confirmation_014"] = confirmation_014.content_sha256
        lock_014 = self.lock_014_loader(self.root / fch.EXPERIMENT_014_LOCK_PATH)
        if any(key not in lock_014 for key in LOCK_014_REQUIRED_KEYS) or lock_014["confirmation_014_sha256"] != confirmation_014.content_sha256 or lock_014["lock_013_sha256"] != lock_013["content_sha256"]:
            raise fch.PhaseError("the Experiment 014 lock does not carry the locked reference states for the frozen 014 confirmation set and the 013 lock")
        ...
        confirmation_015 = atp.load_confirmation(self.root / fch.EXPERIMENT_015_CONFIRMATION_PATH, pool_015, digests)
        digests["confirmation_015"] = confirmation_015.content_sha256
        lock_015 = self.lock_015_loader(self.root / fch.EXPERIMENT_015_LOCK_PATH)
        if any(key not in lock_015 for key in LOCK_015_REQUIRED_KEYS) or lock_015["confirmation_015_sha256"] != confirmation_015.content_sha256 or lock_015["lock_014_sha256"] != lock_014["content_sha256"]:
            raise fch.PhaseError("the Experiment 015 lock does not carry the locked reference states for the frozen 015 confirmation set and the 014 lock")
        ...
        extract = fch.load_inherited_extract(self.root / fch.INHERITED_015_EXTRACT_RELATIVE_PATH, digests=digests, expected_size=fch.EXPECTED_EXTRACT_SIZE_015)
        digests["extract_015"] = extract["content_sha256"]
        return pool, pool_010, lock_011, lock_012, lock_015, extract, digests

    def _inputs(self):
        pool, pool_010, lock_011, lock_012, lock_015, extract, digests = self._base_inputs()
        if not self.confirmation_path.exists() or not self.tracked(self.confirmation_path):
            raise fch.PhaseError("the Experiment 016 confirmation set must be frozen and committed before any scientific phase")
        confirmation = fch.load_confirmation(self.confirmation_path, pool, digests)
        digests = dict(digests) | {"confirmation_016": confirmation.content_sha256}
        return pool, pool_010, lock_011, lock_012, lock_015, extract, confirmation, digests

    def validate(self) -> int:
        try:
            pool, pool_010, lock_011, lock_012, lock_015, extract, digests = self._base_inputs()
        except (fch.PhaseError, ValueError) as error:
            self.log(f"validation failed: {error}")
            return 1
        self.log(f"exposed pool: {len(pool.frames)} frames, {len(pool.tokens)} tokens; 015 extract {len(extract['entries'])} pairs with {len(extract['stage1_state_digests'])} stage-1 state digests; 015 lock {digests['lock_015'][:12]}…")
    ...
    def freeze_confirmation(self) -> int:
        ...
        if self.confirmation_path.exists():
            ...
            return 1
        digest = fch.freeze_confirmation(self.confirmation_path, self.tokenizer_loader(PYTHIA_70M), pool, digests)
    ...
    def _state_for(self, phase: str, digests: Mapping[str, str]) -> dict[str, Any]:
        ...
        if self.results_path.exists():
            state = fch.load_results_state(self.results_path)
            if provenance["git_dirty"] or not pm._COMMIT_SHA.fullmatch(provenance["protocol_code_commit"]):
                raise fch.PhaseError("scientific execution requires a clean Git tree at a committed SHA")
            if fch.state_digests(state) != fch.digest_tuple(digests) or dict(state["versions"]) != provenance["versions"]:
                raise fch.PhaseError("frozen inputs or dependency versions differ from the recorded run")
        else:
            if phase != "explore":
                raise fch.PhaseError(f"{phase} requires an existing results state from explore")
            state = fch.new_results_state(digests=digests, **provenance)
        fch.assert_phase_allowed(phase, state)
        return state
    ...
    def _record_incident(self, state: dict[str, Any], phase: str, error: Exception) -> None:
        ...
        fch.write_results_state(self.results_path, state)
        self.log(f"INCIDENT ({phase}): {error}")

    def explore(self) -> int:
        pool, pool_010, lock_011, lock_012, lock_015, extract, confirmation, digests = self._inputs()
        ...
        if incidents and incidents[-1]["commit"] == commit:
            raise fch.PhaseError("an explore incident is recorded at this commit; a committed fix or documented amendment is required before explore runs again")
        ...
        fch.assert_confirmation_untouched(state, confirmation)
        fch.write_results_state(self.results_path, state)
        seed_runtime(fch.RUNTIME_SEED, PYTHIA_70M.deterministic_algorithms)
        model = self.model_loader(PYTHIA_70M)
        try:
            try:
                fch.run_exploration(model, pool, pool_010, lock_011=lock_011, lock_012=lock_012, lock_015=lock_015, inherited_extract=extract, state=state, results_path=self.results_path, log=self.log)
        ...
        digest = fch.write_results_state(self.results_path, state)
        summary = state["exploration"]["summary"]
        f = cd._f
        self.log(f"explore complete: exposed Spearman {f(summary['spearman'], 3)}, R² {f(summary['r2'], 3)}, entry R² {f(summary['entry_r2_1'], 3)} / {f(summary['entry_r2_2'], 3)} (per-frame min {f(summary['frame_minimum_layer2'], 3)}); scale-only layer 2 {f(summary['scale_only_entry_r2_2'], 3)}; results sha256 {digest}")
        return 0
    ...
    def _predictions(self, weights: pm.Weights, lw: lc.LayerWeights, programs: Mapping[int, atp.LayerProgram], source: Mapping[str, Any], lock_012: Mapping[str, Any], confirmation: fch.Confirmation016, pool: Any) -> dict[str, Any]:
        ...
        fcm = fch.model_from_locks(source, lock_012, lw, programs, pool.reference_ids, plural_ids)
        return fch.lock_predictions(fcm, weights, source["locked_states"], pool, confirmation, source["defined_templates"])
    ...
    def lock(self) -> int:
        ...
        if changed is None:
            raise fch.PhaseError("the explore commit is not an ancestor of the current commit; lock must be written at the explore protocol")
        scientific = [path for path in changed if path.startswith(fch.SCIENTIFIC_PATH_PREFIXES) and path not in fch.NON_SCIENTIFIC_PATHS and not path.startswith(fch.NON_SCIENTIFIC_PREFIXES)]
        if scientific:
            raise fch.PhaseError(f"scientific paths changed since explore: {scientific}; lock must be written at the explore protocol")
        ...
        lock = fch.build_candidate_lock(state=state, digests=digests, confirmation=confirmation, predictions=predictions, protocol_code_commit=self._provenance()["protocol_code_commit"])
        ...
        predictions_text = fch.render_predictions(lock)
        ...
        fch.write_results_state(self.results_path, state)
        self.log(f"candidate lock written to {candidate_path} (sha256 {lock['content_sha256']}) and {predictions_path} (sha256 {state['lock']['predictions_sha256']}). Install both as {fch.LOCK_RELATIVE_PATH} and {fch.PREDICTIONS_RELATIVE_PATH} and commit before confirm.")
        return 0

    def confirm(self) -> int:
        pool, pool_010, lock_011, lock_012, lock_015, extract, confirmation, digests = self._inputs()
        state = self._state_for("confirm", digests)
        lock_path, predictions_path = self.root / fch.LOCK_RELATIVE_PATH, self.root / fch.PREDICTIONS_RELATIVE_PATH
        if not lock_path.exists() or not predictions_path.exists():
            raise fch.PhaseError("missing committed lock or predictions artifact")
        lock = json.loads(lock_path.read_text(encoding="utf-8"))
        git = self.git_state()
        fch.validate_lock(lock, state=state, digests=digests, confirmation=confirmation, predictions_text=predictions_path.read_text(encoding="utf-8"), git_state=git,
        ...
        if recorded_runtime is not None and {k: v for k, v in runtime.items() if k != "seed"} != {k: v for k, v in recorded_runtime.items() if k != "seed"}:
            raise fch.PhaseError(f"the confirm runtime {runtime} differs from the explore runtime {recorded_runtime}; the re-captured reference states must be bitwise reproducible")
        seed_runtime(fch.RUNTIME_SEED, PYTHIA_70M.deterministic_algorithms)
        model = self.model_loader(PYTHIA_70M)
        try:
            weights, lw, programs = pm.Weights.from_model(model), lc.LayerWeights.from_model(model), atp.programs_from_model(model)
            reproduced = fch.assert_lock_predictions_reproduced(lock, self._predictions(weights, lw, programs, lock, lock_012, confirmation, pool))  # PhaseError before anything fresh runs
            ...
            fch.write_results_state(self.results_path, state)
            try:
                stage1 = fch.stage_one(model, pool, pool_010, confirmation, lock, lock_012, protocol_code_commit=commit, log=self.log)
                ...
                fch.write_results_state(self.results_path, state)
                results = fch.stage_two(model, pool, pool_010, confirmation, lock, lock_012, state["confirmation"]["stage1"], log=self.log)
            except pm.IncidentError as error:
    ...
    def report(self) -> int:
        self._inputs()
        state = fch.load_results_state(self.results_path)
        fch.assert_phase_allowed("report", state)
        self.report_path.parent.mkdir(parents=True, exist_ok=True)
        self.report_path.write_text(fch.render_report(state), encoding="utf-8")
