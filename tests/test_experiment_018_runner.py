...
def _populate(root: Path, monkeypatch, fake_world):
    ...
    (root / bc.INHERITED_017_EXTRACT_RELATIVE_PATH).parent.mkdir(parents=True, exist_ok=True)
    (root / bc.INHERITED_017_EXTRACT_RELATIVE_PATH).write_text(extract_text, encoding="utf-8")
    monkeypatch.setattr(cs, "STAGE_UNINFORMATIVE_FLOOR", 0.0)
    _relax_float32_identities(monkeypatch)
    return root, manifest, pool, fake_lock_011, fake_lock_012, fake_locks
