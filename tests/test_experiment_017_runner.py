...
def fake_world(tmp_path_factory):
    """Built once per module (immutable: the manifest, the pool, the fake 011, 012 and 014–016 locks — copied by the loaders — and the extract's JSON text): the fake's Experiment 016 extract over the 7764 recorded pairs."""
    ...
    texts = {}
    texts[hp.INHERITED_016_EXTRACT_RELATIVE_PATH] = pm.canonical_json(hp.inherited_extract_payload(entries, source={"path": "fake"}, digests=digests | {"lock_016": "a" * 64}, stage1_state_digests=stage1_digests)) + "\n"
    return manifest, pool, fake_lock_011, fake_lock_012, fake_lock_014, fake_lock_015, fake_lock_016, texts
...
def _populate(root: Path, monkeypatch, fake_world):
    ...
    manifest, pool, fake_lock_011, fake_lock_012, fake_lock_014, fake_lock_015, fake_lock_016, texts = fake_world
    _copy_repo_files(root)
    for relative, text in texts.items():
        (root / relative).parent.mkdir(parents=True, exist_ok=True)
        (root / relative).write_text(text, encoding="utf-8")
    monkeypatch.setattr(cs, "STAGE_UNINFORMATIVE_FLOOR", 0.0)
    return root, manifest, pool, fake_lock_011, fake_lock_012, fake_lock_014, fake_lock_015, fake_lock_016
