"""Phase isolation, the confirmation freeze, the no-forward-pass lock, the two-stage confirm with its digest barrier and frame guard, and the full state machine for the Experiment 016 runner on a six-layer fake."""
...
def _load_runner_module():
    spec = importlib.util.spec_from_file_location("experiment_016_runner", ROOT / "experiments/016-frame-conditioned-qk/run.py")
...
def fake_world(tmp_path_factory):
    """Built once per module (immutable: the manifest, the pool, the fake 011, 012, 014 and 015 locks — copied by the loaders — and the extract's JSON text): the fake's Experiment 015 extract over the 6180 recorded pairs."""
    ...
    with pytest.MonkeyPatch.context() as patch:
        ...
        stage1_digests = {frame.frame_id: ap.state_digest(atp.locked_state(states[frame.frame_id])) for frame in confirmation_015.frames}
        ...
        # The fake's Experiment 015 extract, the pairs 015 recorded: Experiment 014's 4884 (013's 3732 plus the 014 tokens in the 48 frames of 014's pool) plus the 015 tokens in all 54 frames.
        ...
        for frame in pool.frames:
            ...
            old_frame = frame.frame_id not in frames_012 and frame.frame_id not in frames_013 and frame.frame_id not in frames_014 and frame.frame_id not in frames_015
            if frame.frame_id in frames_015:
                names = [name for name, _ in pool.tokens if name in words_015]
            else:
                names = [name for name, _ in pool.tokens if name in words_015 or name in words_014 or (name in words_013 and frame.frame_id not in frames_014) or (name in words_012 and frame.frame_id in frames_012)
                         or (name not in words_012 and name not in words_013 and name not in words_014 and name not in words_015 and old_frame)]
            ...
            for name in names:
                ...
                if analysis is not None:
                    entries[f"{name}|{frame.frame_id}"] = fch.extract_entry(analysis)
    assert len(entries) == fch.EXPECTED_EXTRACT_SIZE_015
    texts = {}
    texts[fch.INHERITED_015_EXTRACT_RELATIVE_PATH] = pm.canonical_json(fch.inherited_extract_payload(entries, source={"path": "fake"}, digests=digests | {"lock_015": "b" * 64}, stage1_state_digests=stage1_digests)) + "\n"
    return manifest, pool, fake_lock_011, fake_lock_012, fake_lock_014, fake_lock_015, texts
...
def _populate(root: Path, monkeypatch, fake_world):
    ...
    manifest, pool, fake_lock_011, fake_lock_012, fake_lock_014, fake_lock_015, texts = fake_world
    _copy_repo_files(root)
    for relative, text in texts.items():
        (root / relative).parent.mkdir(parents=True, exist_ok=True)
        (root / relative).write_text(text, encoding="utf-8")
    monkeypatch.setattr(cs, "STAGE_UNINFORMATIVE_FLOOR", 0.0)
    return root, manifest, pool, fake_lock_011, fake_lock_012, fake_lock_014, fake_lock_015
...
def explored_world(tmp_path_factory, fake_world):
    ...
    with pytest.MonkeyPatch.context() as patch:
        ...
        with pytest.raises(fch.PhaseError, match="incident"):
            runner.explore()
...
def make_runner(sandbox, monkeypatch, *, logs=None):
    root, manifest, pool, fake_lock_011, fake_lock_012, fake_lock_014, fake_lock_015 = sandbox
    ...
    runner = runner_module.Runner(root=root, results_path=root / "outputs/experiment-016/results.json", report_path=root / "outputs/experiment-016/report.md",
                                  model_loader=lambda spec: make_fake_model(), tokenizer_loader=lambda spec: toy_tokenizer_016(manifest, pool),
                                  ...
                                  lock_015_loader=lambda path: dict(fake_lock_015),
...
def test_full_state_machine_lock_without_forward_pass_and_stage_barrier(explored_sandbox, monkeypatch):
    ...
    pool, pool_010, lock_011, lock_012, lock_015, extract, confirmation, digests = runner._inputs()
    assert len(pool.tokens) == 183 and len(pool.frames) == 54 and len(confirmation.tokens) == 24 and len(confirmation.exposed_frame_prompts) == 24 * 54 and len(confirmation.frames) == 12
    ...
    state = fch.load_results_state(runner.results_path)
    ...
    assert state["phases"]["explore"]["status"] == "complete" and exploration["replication"]["experiment_015"]["max_abs_deviation"] == 0.0
    assert len(exploration["pairs"]) == fch.EXPECTED_EXTRACT_SIZE_015 and set(exploration["locked_states"]) == {frame.frame_id for frame in pool.frames} and all(set(s) == {"p_c", "x1_all", "x2_all"} for s in exploration["locked_states"].values())
    assert exploration["program"]["1"]["rotary_dim"] == 0 and exploration["exposed_check"]["n_tokens"] == 183 and "scale_only_margin" in exploration["exposed_check"]["pairs"] and "ablation_cost_layer2" in exploration["summary"]
    assert all(value < 1e-4 for value in exploration["identities"].values()) and exploration["identities"]["level1_recovery"] < 1e-9
    assert not {prompt.key for prompt in confirmation.all_prompts} & set(state["executed_prompt_keys"])
    with pytest.raises(fch.PhaseError):
        runner.explore()
    ...
    assert lock["experiment"] == "016" and len(lock["predictions"]["rows"]) == 24 * 54 and set(lock["predictions"]["rows"][0]) == set(fch.PREDICTION_COLUMNS) and lock["floors"]["frame_guard_r2"] == fch.FRAME_GUARD_R2
    assert lock["lock_015_sha256"] == "b" * 64 and lock["program"]["2"]["n_heads"] == 8 and "token_means" in lock["predictions"]
    ...
    shutil.copy(candidate, runner.root / fch.LOCK_RELATIVE_PATH)
    (runner.root / fch.PREDICTIONS_RELATIVE_PATH).write_text(text, encoding="utf-8")
    runner.changed_paths = lambda commit: ["src/neural_decompiler/frame_channels.py"]
    with pytest.raises(fch.PhaseError, match="scientific paths"):
        runner.confirm()
    runner.changed_paths = lambda commit: [fch.LOCK_RELATIVE_PATH, fch.PREDICTIONS_RELATIVE_PATH, f"{fch.EXPERIMENT_DIR}/README.md", "README.md"]
    seen = {}
    original_stage_two = fch.stage_two
    ...
    monkeypatch.setattr(fch, "stage_two", guarded_stage_two)
    ...
    assert parts[0] in fch.OUTCOME_Y1 and parts[1] in fch.OUTCOME_Y2 and parts[2] in fch.OUTCOME_Y3
    assert len(results["Y1"]["scored_tokens"]) == 24 and all(entry["n_valid_frames"] == 54 for entry in results["Y1"]["tokens"].values())
    ...
    assert "ladder" in results["Y1"]["descriptive"] and results["Y1"]["descriptive"]["ladder"]["decoded_c_L"]["level1_error_max"] < 1e-3
    ...
    assert "## Tier A" in report and "stage 1" in report and "stage 2" in report and "Comparator rule" in report and "ablation ladder" in report and "decoded c_L" in report
...
def test_incidents_are_recorded_and_block_reruns(sandbox, explored_sandbox, monkeypatch):
    ...
    runner, logs = make_runner(explored_sandbox, monkeypatch)
    shutil.copy(runner.results_path.parent / "candidate-lock.json", runner.root / fch.LOCK_RELATIVE_PATH)
    shutil.copy(runner.results_path.parent / "candidate-predictions.md", runner.root / fch.PREDICTIONS_RELATIVE_PATH)
    with pytest.MonkeyPatch.context() as guard:
    ...
    with pytest.raises(fch.PhaseError, match="confirm"):
        runner.confirm()
